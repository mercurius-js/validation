...
const { kAjv, kOpts, kOverrideFieldResolver } = require('../symbols')
...
class JSONSchemaValidator extends Validator {
  ...
  registerArgumentValidation (typeName, fieldName, schemaTypeField, fieldValidation) {
    ...
    for (const argument of schemaTypeField.args) {
      ...
      const argumentValidationSchema = this.buildValidationSchema(argumentType, namedArgumentType, argumentValidation, id)
  ...
  buildInputTypeFieldValidationSchema (typeName, fieldName, schemaTypeField, fieldValidation) {
    ...
    const builtFieldValidationSchema = this.buildValidationSchema(fieldType, namedFieldType, fieldValidation, id)
      ...
      if (!typeName.startsWith('__') && typeof schemaType.getFields === 'function') {
        for (const [fieldName, schemaTypeField] of Object.entries(schemaType.getFields())) {
          ...
          if (typeof schemaTypeField.args !== 'undefined' && Object.keys(schemaTypeField.args).length > 0) {
            ...
            const fieldValidationSchema = this.buildInputTypeFieldValidationSchema(typeName, fieldName, schemaTypeField, fieldValidation)
